...
import { DESTINATIONS, SEARCH_DESTINATIONS } from "@/lib/data";
...
export default function DestinationsPage() {
  ...
  const [filter, setFilter] = useState<"all" | "pilgrimage" | "adventure">("all");
  ...
  // Combine homepage destinations + search-only destinations
  const allDests = [
  ...
  const pilgrimages = filtered.filter(d => d.type === "pilgrimage");
  const adventures = filtered.filter(d => d.type === "adventure");
  ...
  return (
    <div className="relative min-h-screen font-sans transition-colors duration-300" style={{ background: bg }}>
      ...
      <div className="relative z-10 max-w-[1000px] mx-auto px-4 sm:px-8 pb-20">
        ...
        {/* Search bar + filters */}
        <div className="rounded-2xl p-4 sm:p-5 mb-8 transition-colors duration-300" style={{ background: dark ? "#1c1a17" : "#fafaf8", border: `1.5px solid ${border}` }}>
          <div className="flex flex-col sm:flex-row gap-3 sm:gap-4 items-stretch sm:items-center">
            {/* Search input */}
            <div className="flex-1 relative">
              <div className="absolute left-3 top-1/2 -translate-y-1/2 text-base" style={{ color: textMuted }}>🔍</div>
              <input
                value={searchVal}
                ...
                className="w-full pl-10 pr-4 py-3 rounded-xl text-sm font-semibold outline-none transition-all duration-200"
                style={{
              ...
              />
              {searchVal && (
                <button onClick={() => setSearchVal("")} className="absolute right-3 top-1/2 -translate-y-1/2 text-xs font-bold rounded-full w-5 h-5 flex items-center justify-center transition-colors" style={{ background: dark ? "#333" : "#eee", color: textMuted }}>✕</button>
              )}
            </div>

            {/* Category filters */}
            <div className="flex gap-2">
              {([["all", "All"], ["pilgrimage", "🙏 Pilgrimage"], ["adventure", "🏔️ Adventure"]] as const).map(([key, label]) => (
                <button
                  key={key}
                  onClick={() => setFilter(key)}
                  className="px-4 py-2.5 rounded-xl text-xs sm:text-sm font-semibold cursor-pointer transition-all duration-150"
                  style={{
                    background: filter === key ? accent : "transparent",
                    color: filter === key ? "white" : textMuted,
                    border: `1.5px solid ${filter === key ? accent : border}`,
                  }}
                >
                  {label}
                </button>
        ...
        {filtered.length === 0 ? (
          ...
          <>
            {pilgrimages.length > 0 && (
              <div className="mb-10">
                <h2 className="text-lg font-extrabold mb-4 transition-colors" style={{ color: textPrimary }}>🙏 Pilgrimages</h2>
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                  {pilgrimages.map(d => (
                    <Link key={d.slug} href={d.hasPage ? `/${d.slug}` : "#"} className="group block rounded-xl overflow-hidden no-underline transition-all duration-200 hover:shadow-lg hover:-translate-y-1" style={{ border: `1.5px solid ${border}`, background: cardBg }}>
                      ...
                      <div className="p-4">
                        <p className="font-caveat text-sm" style={{ color: accent }}>{d.note}</p>
            ...
            {adventures.length > 0 && (
              <div className="mb-10">
                <h2 className="text-lg font-extrabold mb-4 transition-colors" style={{ color: textPrimary }}>🏔️ Adventures</h2>
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                  {adventures.map(d => (
                    <Link key={d.slug} href={d.hasPage ? `/${d.slug}` : "#"} className="group block rounded-xl overflow-hidden no-underline transition-all duration-200 hover:shadow-lg hover:-translate-y-1" style={{ border: `1.5px solid ${border}`, background: cardBg }}>
                      <div className="aspect-[16/9] relative" style={{ background: d.grad }}>
